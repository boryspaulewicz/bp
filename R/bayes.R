#' Dopasowuje odporny mieszany model logistyczny
#'
...
#' @param chains Liczba równoległych próbników - domyślnie liczba rdzeni.
...
#' zależną.
#' @param y_sigma Odchylenie standardowe rozkładu t modelującego
#' zmienną zależną. To jest wartość arbitralna - domyślnie przyjęto
...
#' przecięcia są skrajne, domyślna wartość może być nieodpowiednia.
#' @return Wynik próbkowania zwrócony przez funkcję stan z dodanym
#' atrybutem fixed.effects, zawierającym nazwy kolumn macierzy modelu.
#' @export
robust.mixed.logistic = function(fixed, random, d,
    y_nu = 4, y_sigma = 1.548435, ranef_nu = 4, beta_sigma = 20,
    chains = parallel::detectCores(), pars = NULL, ...){
    ...
    fit = stan(paste(path.package('bpaulewicz'), 'stan_models/robust_mixed_logistic.stan', sep = '/'),
        data = list(D = ncol(X), R = ncol(Z), N = nrow(X), I = max(id),
            X = X, Z = Z, n = d$n, y = y, id = id,
            y_nu = y_nu, y_sigma = y_sigma, ranef_nu = ranef_nu, beta_sigma = beta_sigma),
